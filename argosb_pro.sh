...
echo '@reboot /bin/bash -c "nohup ./aspro/cloudflared tunnel --url http://localhost:$(grep "listen_port" ./aspro/sb.json | grep -oP '\''\\d+'\'' | sed -n '\''2p'\'') --edge-ip-version auto --no-autoupdate --protocol http2 > ./as/argo.log 2>&1 & pid=\$! && echo \$pid > ./as/sbargopid.log"' >> /tmp/crontab.tmp
fi
fi
