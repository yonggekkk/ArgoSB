...
# 选填！每个账号对应名称空一格，留空自动生成
APP_NAMES=""

# 选填！argo固定隧道端口，每个账号对应空一格，留空表示不启用argo
VMPTS="" 

# 选填！argo固定隧道域名，每个账号对应空一格，留空表示不启用argo
AGNS="" 

# 选填！argo固定隧道token，每个账号对应空一格，留空表示不启用argo
AGKS=""
...
pushout() {
  if echo "$push_out" | grep -iq "insufficient"; then
    ...
    echo "🔴第 $((i+1)) 个实例部署：${APP_NAME} 失败了，SAP繁忙中！或自查参数设置是否有误，再试一下"
    return 1
  else
    echo "${APP_NAME} 完成"
    return 0
...
sapcfevn() {
  ...
  if [[ -n "$AGN" && -n "$AGK" && -n "$VMPT" ]]; then
    cf set-env "$APP_NAME" agn "$AGN"
    cf set-env "$APP_NAME" agk "$AGK"
    cf set-env "$APP_NAME" vmpt "$VMPT"
    cf set-env "$APP_NAME" argo "y"
    else
    cf unset-env "$APP_NAME" agn
    cf unset-env "$APP_NAME" agk
    cf unset-env "$APP_NAME" vmpt
    cf unset-env "$APP_NAME" argo
  fi
...
for i in "${!CF_USERNAMES[@]}"; do
  ...
  if [ -z "$APP_NAMES" ]; then
    ...
    APP_NAME="${APP_NAMES[$i]//[^a-zA-Z0-9_-]/-}"
  fi
  echo "=============================================="
  echo "=========第 $((i+1)) 个实例开始启动============"
  echo "=============================================="
